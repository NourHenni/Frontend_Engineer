...
const Matieres = () => {
  ...
  const [currentMatiereEvaluations, setCurrentMatiereEvaluations] =
    useState(null);
  const [pollingIntervals, setPollingIntervals] = useState({});
  const [evaluationModalVisible, setEvaluationModalVisible] = useState(false);
  const [selectedMatiereForEvaluation, setSelectedMatiereForEvaluation] =
    useState(null);
      ...
      const proposals = response.data.historiquePropositions
        ...
        .map((p) => ({
          ...
          enseignant: p.enseignant
            ...
            : null,
          dateFormatted: new Date(p.dateProposition).toLocaleDateString(
            "fr-FR",
            {
              day: "2-digit",
              month: "long",
  ...
  const handleEvaluationSubmit = async (values) => {
    try {
      ...
      setLiveCounts((prev) => ({
        ...prev,
        [selectedMatiereForEvaluation._id]:
          (prev[selectedMatiereForEvaluation._id] || 0) + 1,
      }));
    } catch (err) {
      message.error(
  ...
  const loadEvaluations = async (matiereId) => {
    try {
      ...
      message.error(
        err.response?.data?.message || "Erreur de chargement des évaluations"
      );
    }
  };
              ...
              {record.historiquePropositions?.filter((p) => !p.valide).length >
                ...
                <span
                  className={`proposal-badge ${
                    newProposals[record._id] ? "new-proposal" : ""
                  }`}
                >
                  {
                    record.historiquePropositions.filter((p) => !p.valide)
                      .length
                  }
                </span>
              )}
  ...
  const showDetails = async (record) => {
    try {
      console.log(
        "Détails matière - Historique:",
        record.historiqueModifications
      );

      const data = await fetchMatiereById(record._id); // Corrigé ici
      ...
      const historyColumns = [
        {
          ...
          sorter: (a, b) =>
            new Date(a.dateModification) - new Date(b.dateModification),
        },
        {
          title: "Modifications",
          key: "modifications",
        ...
        content: (
          <div className="matiere-details">
            ...
            <div className="infos-grid">
              ...
              {userRole === "admin" && (
                <div>
                  <strong>Enseignant:</strong> {getEnseignantName()}
                </div>
              )}
              <div>
            ...
            {(userRole === "admin" || userRole === "enseignant") && (
              <>
                ...
                <Table
                  ...
                  scroll={{ y: 240 }}
                  bordered
                  locale={{ emptyText: "Aucune modification enregistrée" }}
                />
              </>
            )}
          </div>
                ...
                {(sections, { add: addSection, remove: removeSection }) => {
                  ...
                  const updateChapitreStatus = (sections) => {
                    ...
                    const currentSections =
                      formValues.Curriculum[chapitreIndex].sections || [];

                    let newStatus = "NonCommencee";
                    if (
                      currentSections.some(
                        (s) => s.AvancementSection === "EnCours"
                      )
                    ) {
                      newStatus = "EnCours";
                    } else if (
                      currentSections.every(
                        (s) => s.AvancementSection === "Terminee"
                      )
                    ) {
                      newStatus = "Terminee";
                    }
                            ...
                            <Select
                              onChange={() => {
                                ...
                                setTimeout(() => {
                                  const formValues = form.getFieldsValue();
                                  const currentSections =
                                    formValues.Curriculum[chapitreIndex]
                                      .sections || [];
  ...
  <div className="form-section">
    ...
    >
      <Select
        ...
        placeholder={
          loadingEnseignants ? "Chargement..." : "Sélectionnez un enseignant"
        }
      >
        ...
        {!isCurriculumEdit ? (
          ...
          <>
            ...
            <div className="form-section">
              ...
              >
                <Input placeholder="Ex: MTH101" disabled={isEnseignant} />
              </Form.Item>
              ...
              <Form.Item
                ...
                rules={[{ required: true, message: "Champ obligatoire" }]}
              >
                <Input placeholder="Ex: GM1" disabled={isEnseignant} />
              </Form.Item>
            </div>
          ...
          <Form.Item
            ...
            label="Curriculum"
            rules={[
              { required: true, message: "Le curriculum est obligatoire" },
            ]}
          >
            <Form.List name="Curriculum">
              {(chapitres, { add: addChapitre, remove: removeChapitre }) => (
    ...
    <div className="matieres-page">
      ...
      <div className="content-container">
        ...
        <Modal
          ...
          destroyOnClose
        >
          <Form form={form} layout="vertical" onFinish={handleCurriculumSubmit}>
            {renderCurriculumForm()}
            <Form.Item>
            ...
            {selectedProposal && (
              <div>
                ...
                <div className="teacher-info">
                  <h5>
                    {selectedProposal.enseignant?.nom}{" "}
                    {selectedProposal.enseignant?.prenom}
                    <br />
                  </h5>
                ...
                <Table
                  ...
                  dataSource={Object.entries(selectedProposal.contenu)
                    .filter(([key, newValue]) => {
                      const originalMatiere = data.find(
                        (m) => m._id === selectedProposal.matiereId
                      );
                ...
                {selectedProposal.contenu.Curriculum &&
                  JSON.stringify(selectedProposal.contenu.Curriculum) !==
                    JSON.stringify(
                      data.find((m) => m._id === selectedProposal.matiereId)
                        .Curriculum
                    ) && (
                    <>
                      <h3 style={{ marginTop: 20 }}>
                        Changements dans le Curriculum :
                      </h3>
                      <div style={{ maxHeight: 400, overflow: "auto" }}>
                        {(selectedProposal.contenu.Curriculum || []).map(
                          (chapitre, idx) => {
                            const originalChapitre = data.find(
                              (m) => m._id === selectedProposal.matiereId
                            ).Curriculum[idx];
                            ...
                            return (
                              <div key={idx} style={{ marginBottom: 15 }}>
                                <h4>
                                  Chapitre {idx + 1}: {chapitre.titreChapitre}
                                </h4>
                                {chapitre.sections?.map((section, sIdx) => {
                                  const originalSection =
                                    originalChapitre?.sections?.[sIdx];

                                  return (
                                    <div key={sIdx} style={{ marginLeft: 15 }}>
                                      {(!originalSection ||
                                        section.nomSection !==
                                          originalSection.nomSection ||
                                        section.Description !==
        ...
        <Modal
          title={`Évaluations - ${
            currentMatiereEvaluations?.matiere?.Nom || ""
          }`}
            ...
            <>
              <div style={{ marginBottom: 20 }}>
                ...
                {currentMatiereEvaluations.matiere.Enseignant && (
                  <p>
                    Enseignant:{" "}
                    {currentMatiereEvaluations.matiere.Enseignant.nom}{" "}
                    {currentMatiereEvaluations.matiere.Enseignant.prenom}
                  </p>
                )}
                <p>
                  Nombre d'évaluations:{" "}
                  {currentMatiereEvaluations.nombreEvaluations}
                </p>
              </div>
                ...
                columns={[
                  {
                    ...
                    dataIndex: "VolumeHoraire",
                    key: "VolumeHoraire",
                    render: (note) => getEvaluationLabel(note),
                  },
                  {
                    title: "Méthodes Pédagogiques",
                    dataIndex: "MethodesPedagogiques",
                    key: "MethodesPedagogiques",
                    render: (note) => getEvaluationLabel(note),
                  },
                  {
                    title: "Objectifs",
                  ...
                  {
                    title: "Cohérence",
                    dataIndex: "CoheranceContenu",
                    key: "CoheranceContenu",
                    render: (note) => getEvaluationLabel(note),
                  },
                  {
                    title: "Satisfaction",
                  ...
                  {
                    title: "Pertinence",
                    dataIndex: "PertinenceMatiere",
                    key: "PertinenceMatiere",
                    render: (note) => getEvaluationLabel(note),
                  },
                  {
                    title: "Remarques",
