...
const Profile = () => {
  ...
  // ─── Fonctions utilitaires pour gérer les listes CV ────────────────────────
  const handleRemoveCvItem = (field, indexToRemove) => {
  ...
  const handleRemoveExperience = (indexToRemove) => {
    const currentExps = form.getFieldValue(["cvinfos", "experiences"]) || [];
    const newExps = currentExps.filter((_, index) => index !== indexToRemove);
    form.setFieldsValue({
      cvinfos: {
        ...(form.getFieldValue("cvinfos") || {}),
        experiences: newExps,
    ...
    try {
      ...
      const updateData = {
        ...
        dateDeNaissance: values.dateDeNaissance
          ? values.dateDeNaissance.format("YYYY-MM-DD")
          : null,
        cvinfos: {
    ...
    return (
      ...
      >
        {pfas.length > 0 ? (
          <Table
            columns={[
              { title: "Titre", dataIndex: "titreSujet", key: "title" },
              { title: "code_pfa", dataIndex: "code_pfa", key: "code_pfa" },
              {
                title: "Description",
              ...
              { title: "Année", dataIndex: "annee", key: "year" },
              {
                title: "Statut",
                dataIndex: "status",
                key: "status",
        ...
        <div style={{ marginBottom: 16 }}>
          {values.map((item, index) => (
            <Tag
              key={`${field}-${index}`}
              closable={editing}
              onClose={() => handleRemoveCvItem(field, index)}
  ...
  const renderAcademicStatuses = () => {
    const { academic_statuses } = user || {};

    return (
      <Card
        title="Statuts Académiques"
        style={cardStyle}
        headStyle={headerStyle}
      >
          ...
          <div>
            {academic_statuses.map((status, index) => (
              <div key={status._id} style={{ marginBottom: 16 }}>
                <div>
                  <strong>Année académique:</strong> {status.academic_year}
                </div>
                <div>
                ...
                {status.details && (
                  <div>
                    <strong>Détails:</strong> {status.details}
                  </div>
                )}
            ...
            {inputVisible.experiences ? (
              <Space direction="vertical" style={{ width: "100%" }}>
                ...
                <Space>
                  ...
                  <Button
                    onClick={() =>
                      setInputVisible({ ...inputVisible, experiences: false })
                    }
                  >
                    Cancel
                  </Button>
                      ...
                      <Col xs={24} md={12}>
                        ...
                        >
                          ...
                          <Form.Item name="cin" label="CIN">
                            <Input disabled={true} style={disabledFieldStyle} />
                          </Form.Item>
                        </Card>
                      ...
                      <Col xs={24} md={12}>
                        ...
                        >
                          <Form.Item name="addresse" label="Adresse complète">
                            <AntdTextArea rows={2} disabled={!editing} />
                          </Form.Item>
                          <Row gutter={16}>
                            <Col span={12}>
