    ...
    Modal.confirm({
      ...
      content: (
        <div>
          <p style={{ color: 'red', marginBottom: 16 }}>
            Warning: Creating a new academic year will archive all current PFAs and summer internships.
            You won't be able to view them in the current interface after this action.
          </p>
      ...
      onOk: async () => {
        ...
        if (!academicYear || !/^\d{4}-\d{4}$/.test(academicYear)) {
          message.error('Please enter a valid academic year in YYYY-YYYY format');
        ...
        if (endYear !== startYear + 1) {
          message.error('Academic year must be exactly one year (e.g., 2025-2026)');
          return Promise.reject();
        }
        ...
        try {
          ...
          message.success(`New academic year ${academicYear} created successfully.`);
        ...
        <Tabs defaultActiveKey="1" style={{ marginBottom: 24 }}>
          <TabPane tab="Students" key="1">
            <Card 
              title="Students List" 
              bordered={false} 
              style={{ height: "auto", overflow: "hidden", marginBottom: "20px" }}
            >
