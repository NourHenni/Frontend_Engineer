...
// ─── Sous-composant : Informations personnelles ───────────────────────────────
const PersonalInfoSection = ({ student, editMode, form, renderField }) => {
  if (editMode) {
    return (
      <>
  ...
  return (
    <Descriptions column={1} labelStyle={{ fontWeight: 600, color: "#555" }}>
      <Descriptions.Item label="CIN">
        {renderField(student.cin)}
      </Descriptions.Item>
      <Descriptions.Item label="Gender">
        {renderField(student.genre)}
      </Descriptions.Item>
...
// ─── Sous-composant : Informations de contact ─────────────────────────────────
const ContactInfoSection = ({ student, editMode, form, renderField }) => {
  if (editMode) {
    return (
      <>
  ...
  return (
    <>
      <AntdText>
        ...
        {renderField(student.telephone)}
      </AntdText>
      <AntdText>
        <MailOutlined style={{ color: "#1890ff" }} />{" "}
        {renderField(student.adresseEmail)}
      </AntdText>
      <AntdText>
        <HomeOutlined style={{ color: "#1890ff" }} />{" "}
        {renderField(student.addresse)}
      </AntdText>
    </>
  );
};

// ─── Sous-composant : Informations académiques ────────────────────────────────
const AcademicInfoSection = ({ student, editMode, form, renderField }) => {
  if (editMode) {
    return (
      <>
        <Form.Item label="Université" name="universite">
          <Input />
        </Form.Item>
        <Form.Item label="Etablissement" name="etablissement">
          <Input />
        </Form.Item>
        <Form.Item label="Type Licence" name="type_licence">
          <Input />
        </Form.Item>
        <Form.Item label="Spécialité" name="specialite">
          <Input />
        </Form.Item>
        <Form.Item label="Année Licence" name="annee_licence">
          <Input />
        </Form.Item>
        <Form.Item label="Année Bac" name="annee_bac">
          <Input />
        </Form.Item>
        <Form.Item label="Moyenne Bac" name="moyenne_bac">
          <Input />
        </Form.Item>
        <Form.Item label="Mention" name="mention">
          <Input />
        </Form.Item>
        <Form.Item label="Baccalauréat" name="baccalaureat">
          <Input />
        </Form.Item>
        <Form.Item label="Année entrée ISAMM" name="annee_entree_isamm">
          <Input />
        </Form.Item>
      </>
    );
  ...
  return (
    <Descriptions column={1} labelStyle={{ fontWeight: 600, color: "#555" }}>
      <Descriptions.Item label="University">
        {renderField(student.universite)}
      </Descriptions.Item>
      <Descriptions.Item label="Establishment">
        {renderField(student.etablissement)}
      </Descriptions.Item>
      <Descriptions.Item label="Degree Type">
        {renderField(student.type_licence)}
      </Descriptions.Item>
      <Descriptions.Item label="Specialty">
        {renderField(student.specialite)}
      </Descriptions.Item>
      <Descriptions.Item label="Year of Study">
        {renderField(student.annee_licence)}
      </Descriptions.Item>
      <Descriptions.Item label="Baccalaureate Year">
        {renderField(student.annee_bac)}
      </Descriptions.Item>
      <Descriptions.Item label="Baccalaureate Average">
        {renderField(student.moyenne_bac)}
      </Descriptions.Item>
      <Descriptions.Item label="Mention">
        {renderField(student.mention)}
      </Descriptions.Item>
      <Descriptions.Item label="Baccalaureate Field">
        {renderField(student.baccalaureat)}
      </Descriptions.Item>
      <Descriptions.Item label="Entry Year">
        {renderField(student.annee_entree_isamm)}
      </Descriptions.Item>
    </Descriptions>
  );
};
...
function StudentDetails() {
  ...
  // Fonction utilitaire pour afficher les champs manquants
  const renderField = (field) =>
    !field && field !== 0 ? (
      <AntdText type="secondary">Unavailable</AntdText>
    ) : (
      <AntdText>{field}</AntdText>
    );
    ...
    if (!student?.academic_statuses || student.academic_statuses.length === 0) {
      return (
        <Card title="Academic Statuses" bordered={false} style={cardStyle}>
          <AntdText type="secondary">No academic statuses recorded</AntdText>
        </Card>
      );
    }
