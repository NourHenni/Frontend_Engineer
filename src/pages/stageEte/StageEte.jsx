...
function StageEte() {
  ...
  const fetchStages = useCallback(async () => {
    if (userRole === "enseignant") {
      ...
      try {
        ...
        const stagesData = Array.isArray(response)
          ? response
        ...
        message.error(
          error.response?.data?.message ||
            "Erreur lors du chargement des stages assignés"
        );
      } finally {
  ...
  const formFields = [
    ...
    },
    {
      ...
      name: "rapport",
      type: "upload",
      rules: [{ validator: validatePdfAndRequired }],
    },
    {
      label: "Attestation",
      name: "attestation",
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      formData.append(
        "ficheEvaluation",
        values.ficheEvaluation[0].originFileObj
      );
      ...
      setSuccessData({
        ...
        reference:
          response.reference || "REF-" + Math.floor(Math.random() * 10000),
      });
      ...
      message.error(
        error.response?.data?.message || error.message || "Échec du dépôt."
      );
    }
  };
          ...
          <Col span={24}>
            <Alert
              ...
              description={
                <>
                  <p>
                    <InfoCircleOutlined /> Tous les champs sont obligatoires
                  </p>
                  <p>
                    <InfoCircleOutlined /> Les fichiers doivent être au format
                    PDF
                  </p>
                  <p>
                    <InfoCircleOutlined /> Maximum 2 dépôts autorisés par
                    étudiant
                  </p>
                </>
              }
          ...
          <Col span={24}>
            ...
            <Steps current={currentStep} onChange={setCurrentStep}>
              <Step
                ...
                description="Toutes les informations du stage"
              />
              <Step
                title="Upload des documents"
                description="Rapport, attestation et fiche"
              />
  ...
  return (
    <div>
      ...
      <div className="stage-ete-container">{renderContentByRole()}</div>
    </div>
  );
}
