...
function AffectationView() {
  ...
  const fetchAffectation = useCallback(async () => {
    ...
    try {
      ...
      setErrorMsg(error.response?.data?.message || error.message || "Erreur lors de la récupération de l'affectation.");
      setData(null); 
    } finally {
  ...
  return (
    <Card title={`Détail Affectation - ${type === "premiereannee" ? "1ère Année" : "2ème Année"}`} bordered>
      ...
      {errorMsg && <Result status="error" title="Erreur lors du rafraîchissement" subTitle={errorMsg} style={{ marginBottom: 16 }}/>}
      
      <Descriptions column={1} bordered>
        <Descriptions.Item label="Titre du sujet">{stage.titreSujet || "N/A"}</Descriptions.Item>
        <Descriptions.Item label="Entreprise">{stage.nomEntreprise || "N/A"}</Descriptions.Item>
        <Descriptions.Item label="Description">{stage.description || "N/A"}</Descriptions.Item>
        <Descriptions.Item label="Année du stage">{stage.anneeStage || "N/A"}</Descriptions.Item>
        <Descriptions.Item label="Niveau">{stage.niveau || "N/A"}</Descriptions.Item>
        <Descriptions.Item label="Nature du sujet">{stage.natureSujet || "N/A"}</Descriptions.Item>
        ...
        <Descriptions.Item label="Enseignant Affecté">
          {enseignant ? (
            <>
              {enseignant.nom} {enseignant.prenom} ({enseignant.adresseEmail || enseignant.email || "Email non fourni"})
            </>
          ) : (
        ...
        <Descriptions.Item label="Soutenance">
          {soutenance ? (
            <>
              Jour : {soutenance.jour ? dayjs(soutenance.jour).format('DD/MM/YYYY') : "N/A"} <br />
