...
export const getInternshipsByTypeAndYear = async (type, anneeStage, token) => {
...
export const updateAssignedTeacher = async (
  type,
  stageId,
  teacherId,
  token
...
export const getAssignedStages = async (niveau, token) => {
  const response = await fetch(
    `http://localhost:5000/internship/${niveau}/assign/tome`,
    {
      headers: {
        Authorization: `Bearer ${token}`,
      },
  ...
  if (!response.ok) {
    throw new Error(
      data.message || "Erreur lors du chargement des stages assignés"
    );
  }
  return data.stages;
};
...
export const getMyAffectationByType = async (type, token) => {
  const response = await fetch(
    `http://localhost:5000/internship/monpv/${type}/me`,
    {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.message || "Erreur lors de la récupération du stage"
    );
  }
...
export const updatePeriod = async (id, payload, token) => {
  return await axios.patch(
    `http://localhost:5000/internship/updateperiode/${id}`,
    payload,
    {
      headers: {
        Authorization: `Bearer ${token}`,
